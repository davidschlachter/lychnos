...
func Get(start, end time.Time) []ReportingInterval {
	...
	for d := start; d.Before(end) && d.Before(lastDayThisMonth); d = time.Date(bgtYear, bgtMonth+1, 1, 0, 0, 0, 0, currentLocation) {
